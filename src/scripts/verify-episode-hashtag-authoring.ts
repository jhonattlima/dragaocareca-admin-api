...
const verifyFoundationFocus = async (fixture: Fixture): Promise<void> => {
  ...
  assert.deepEqual(authoringModule.validateGeminiTagCandidates({ candidates: [] }), []);
  assert.equal(authoringModule.validateGeminiTagCandidates({ candidates: [...createCandidates().slice(0, 49), { ...createCandidates()[0], tag: "tag0" }] }).length, 49);
