...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    const ffmpegArgs = ["-hide_banner", "-loglevel", "error", "-protocol_whitelist", "file,pipe", "-loop", "1", "-i", cover, "-i", audio, "-filter_complex", "[1:a]showwaves=s=1280x360:mode=line:colors=white[wave];[0:v]scale=1280:720,format=yuv420p[cover];[cover][wave]overlay=0:360[v]", "-map", "[v]", "-map", "1:a", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-movflags", "+faststart", "-t", sourceDuration.toFixed(3), "-shortest", output];
    ...
    const result = { sourceSha256: await hashFile(audio), sourceBytes: (await fs.stat(audio)).size, sourceDurationSeconds: sourceDuration, outputSha256: await hashFile(output), outputBytes: (await fs.stat(output)).size, outputDurationSeconds: duration, video: { codec: video!.codec_name, pixelFormat: video!.pix_fmt }, audio: { codec: audioStream!.codec_name }, decoded: true };
